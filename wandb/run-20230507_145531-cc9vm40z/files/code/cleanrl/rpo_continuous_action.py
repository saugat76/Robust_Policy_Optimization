...
class Agent(nn.Module):
    def __init__(self, envs, rpo_alpha):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), args.num_nodes)),
            nn.Tanh(),
            layer_init(nn.Linear(args.num_nodes, args.num_nodes)),
            nn.Tanh(),
            layer_init(nn.Linear(args.num_nodes, 1), std=1.0),
        )
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(args.num_nodes, args.num_nodes)),
            nn.Tanh(),
            layer_init(nn.Linear(args.num_nodes, np.prod(envs.single_action_space.shape)), std=0.01),
        )
